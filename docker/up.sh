...
if [ "$subcommand" = check ]; then
  if [ -z "${CLAUDE_CODE_OAUTH_TOKEN:-}" ]; then
    echo "riff: no token, and nothing configured to fetch one." >&2
    echo "  Set RIFF_TOKEN_CMD in $local_env${outside_env:+ or $outside_env}." >&2
    exit 1
  fi
  echo "riff: a token is available (${#CLAUDE_CODE_OAUTH_TOKEN} characters). Nothing was started."
  exit 0
fi
